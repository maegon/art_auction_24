...
//                질문 테스트데이터
           ...
           /* if (member1 == null) {
            ...
            if (!productService.existsByTitleAndArtist("토끼", artist1)) {
                create("토끼", "이 그림은 부드러운 색조와 섬세한 디테일로 그려진 귀여운 토끼를 담고 있습니다. 토끼의 큰 눈과 푹신한 털은 따뜻하고 아늑한 느낌을 주며, 자연 속에서 조용히 쉬고 있는 모습을 표현했습니다. 그림 속 토끼는 순수하고 순진한 매력을 지니고 있어, 보는 이로 하여금 평화롭고 행복한 감정을 느끼게 합니다. 이 작품은 자연과 동물의 아름다움을 경이롭게 담아내려는 저의 노력의 결과입니다.", "색연필", 90, 128, new BigDecimal(10000), LocalDateTime.now(), "/app/resources/static/images/product/product_paint_2.jpg", "수채화", artist1);
            }
            ...
            if (!productService.existsByTitleAndArtist("눈", artist1)) {
                create("눈", "이 작품은 단순한 형태의 눈을 통해 깊은 감정을 표현한 초상화입니다. 섬세하게 그려진 눈동자는 보는 이로 하여금 감정의 미세한 변화를 읽을 수 있게 해줍니다. 눈의 윤곽과 세밀한 디테일은 인간의 내면 깊숙한 곳까지 들여다볼 수 있는 창을 제공합니다. 강렬한 색조와 명암 대비는 눈의 감정적 강도를 강조하며, 간결한 배경은 감상의 집중을 돕습니다. 이 그림은 시각적으로 감정을 전하며, 보는 이에게 깊은 인상을 남깁니다.", "종이에 색연필과 물감", 128, 85, new BigDecimal(60000), LocalDateTime.now(), "/temp/images/product/product_paint_11.jpg", "수채화", artist1);
