...
//                FAQ 테스트데이터 (어드민이작성)
            ...
            if (!memberService.checkUsernameExists("user1")) {
                M2 = memberService.join("", "user1", password, "user1@test.com", "user1",
                        "010-1234-5678", "대전광역시 서구 대덕대로 179 굿모닝어학원빌딩 9층");

//                질문 테스트데이터
                questionService.create(new QuestionForm("ㅁㄴㅇㄴㅁㅇㄴㅁㅇ", "asdfsadf", QuestionType.ARTIST), M2);
                questionService.create(new QuestionForm("ㅁㄴㅇㄴㅁㅇㄴㅁㅇ", "asdfsadf", QuestionType.ARTIST), M2);
                questionService.create(new QuestionForm("ㅁㄴㅇㄴㅁㅇㄴㅁㅇ", "asdfsadf", QuestionType.ARTIST), M2);
                questionService.create(new QuestionForm("ㅁㄴㅇㄴㅁㅇㄴㅁㅇ", "asdfsadf", QuestionType.ARTIST), M2);
                questionService.create(new QuestionForm("ㅁㄴㅇㄴㅁㅇㄴㅁㅇ", "asdfsadf", QuestionType.ARTIST), M2);
           ...
           /* if (member1 == null) {
            ...
            if (!productService.existsByTitleAndArtist("토끼", artist1)) {
                create("토끼", "이 그림은 부드러운 색조와 섬세한 디테일로 그려진 귀여운 토끼를 담고 있습니다. 토끼의 큰 눈과 푹신한 털은 따뜻하고 아늑한 느낌을 주며, 자연 속에서 조용히 쉬고 있는 모습을 표현했습니다. 그림 속 토끼는 순수하고 순진한 매력을 지니고 있어, 보는 이로 하여금 평화롭고 행복한 감정을 느끼게 합니다. 이 작품은 자연과 동물의 아름다움을 경이롭게 담아내려는 저의 노력의 결과입니다.", "색연필", 90, 128, new BigDecimal(10000), LocalDateTime.now(), "product_paint_2.jpg", "수채화", artist1);
            }
            if (!productService.existsByTitleAndArtist("은하계", artist2)) {
                create("은하계", "이 그림은 광활한 우주를 배경으로 한 은하계를 담아낸 작품입니다. 별들이 수놓은 밤하늘과 그 사이를 흐르는 은하의 신비로운 모습이 아름답게 표현되어 있습니다. 색다른 색조와 반짝이는 별빛, 그리고 깊은 우주의 신비로움이 조화를 이루며 관객을 우주의 끝없는 광활함 속으로 초대합니다. 이 작품은 우주의 웅장함과 아름다움을 탐구하고, 무한한 상상의 세계를 경험하게 해주는 저의 창의력의 표현입니다..", "종이에 물감", 600, 300, new BigDecimal(11000), LocalDateTime.now(), "/images/product/product_paint_4.jpg", "수채화", artist2);
            }
            if (!productService.existsByTitleAndArtist("천사와 악마", artist3)) {
                create("천사와 악마", "이 그림은 천사와 악마의 대비를 통해 인간 내면의 갈등과 도덕적 이중성을 탐구한 작품입니다. 천사는 순수함과 희망의 상징으로, 부드럽고 따뜻한 색조로 표현되어 있습니다. 반면, 악마는 어두운 색상과 날카로운 선들로 그려져 있으며, 위협적이고 강렬한 존재감을 드러냅니다. 두 캐릭터의 극명한 대조는 선과 악, 빛과 어둠 간의 끊임없는 싸움을 시각적으로 드라마틱하게 담아내며, 인간의 도덕적 선택과 그에 따른 내적 갈등을 관객에게 깊이 있게 전달합니다.", "종이에 물감", 30, 20, new BigDecimal(12000), LocalDateTime.now(), "/product_paint_3.jpg", "수채화", artist3);
            }
            ...
            if (!productService.existsByTitleAndArtist("눈", artist1)) {
                create("눈", "이 작품은 단순한 형태의 눈을 통해 깊은 감정을 표현한 초상화입니다. 섬세하게 그려진 눈동자는 보는 이로 하여금 감정의 미세한 변화를 읽을 수 있게 해줍니다. 눈의 윤곽과 세밀한 디테일은 인간의 내면 깊숙한 곳까지 들여다볼 수 있는 창을 제공합니다. 강렬한 색조와 명암 대비는 눈의 감정적 강도를 강조하며, 간결한 배경은 감상의 집중을 돕습니다. 이 그림은 시각적으로 감정을 전하며, 보는 이에게 깊은 인상을 남깁니다.", "종이에 색연필과 물감", 128, 85, new BigDecimal(60000), LocalDateTime.now(), "/app/images/product/product_paint_11.jpg", "수채화", artist1);
