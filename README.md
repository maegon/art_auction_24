...
## 🚨 이슈
대표 작품을 수정한 후 동일한 입력란이 중복으로 생성되는 현상이 발생 <br> <br>

## 🛑 원인
정확한 원인은 아직 파악되지 않았으나, 입력 필드를 추가하는 과정에서 문제가 발생한 것으로 추정됨 <br> <br>

## 🚥 해결
데이터베이스에서 중복된 대표 작품명이 존재하는 경우, 하나를 제외한 나머지를 제거하는 방식으로 문제를 해결하였으며, 이를 위해 JS 코드를 수정하여 처리 로직을 구현함.
</br></br>
</details>
...
이번 프로젝트를 진행하면서 정말 많은 것을 배울 수 있었습니다. 무엇보다도 각 팀원들이 맡은 역할을 충실히 수행해 주신 덕분에 목표했던 결과물을 잘 마무리할 수 있었습니다. 모든 팀원분들께 감사의 말씀을 드리고 싶습니다.
